...
public class PacketBUTTON extends Packet {
    ...
    public PacketBUTTON(Character character, String map_name, boolean repeat, boolean down, boolean queue, short amount, byte axis)
	{
		super(PT_BUTTON);
		short code = 0;
		short flags = BT_VKEY;
		flags |= BT_USE_NAME;
		appendPayload(code, map_name , character, repeat, down, queue, amount, axis, flags);
	}
	...
	private void appendPayload(short code, String map_name, char button_name, boolean repeat, boolean down, boolean queue, short amount, byte axis, short flags)
	{
		if(amount>0)
			flags |= BT_USE_AMOUNT;
		else
