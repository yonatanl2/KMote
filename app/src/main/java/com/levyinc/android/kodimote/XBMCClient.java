...
public class XBMCClient 
{
    ...
    public void sendButton(char character, boolean repeat, boolean down, boolean queue, short amount, byte axis) throws IOException
	{
        PacketBUTTON p = new PacketBUTTON(character, "KB", repeat, down, queue, amount, axis);
		p.send(hostAddress, hostPort);
	}
    ...
    public void sendAction(String actionmessage, byte actiontype) throws IOException
    {
        PacketACTION p = new PacketACTION(actionmessage, actiontype);
        p.send(hostAddress, hostPort);
    }
